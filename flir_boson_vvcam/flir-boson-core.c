...
static const struct flir_boson_format * flir_boson_find_format(struct flir_boson_dev *sensor, u32 code) {
	...
	switch (code) {
        ...
        case MEDIA_BUS_FMT_YUYV8_1X16:
			code_search = MEDIA_BUS_FMT_UYVY8_1X16;
			break;
	    case MEDIA_BUS_FMT_Y10_1X10:
	    case MEDIA_BUS_FMT_Y12_1X12:
	    case MEDIA_BUS_FMT_Y16_1X16:
            code_search = MEDIA_BUS_FMT_Y14_1X14;
			break;
		default:
	        code_search = code;
	}
	for (i = 0; i < FLIR_BOSON_NUM_FORMATS; i++) {
		if (flir_boson_formats[i].code == code_search)
...
#endif
{
    ...
    dev_info(dev, "FLIR Boson+ MIPI camera driver START2 probing\n");
    ...
    if (flir_boson_get_int_val(sensor, BOSON_GETCAMERASN, &sensor->camera_sn) == R_SUCCESS) dev_info(dev, "Camera SN: 0x%08X", sensor->camera_sn);
    else dev_warn(dev, "Could not read camera serial number");

    if (flir_boson_send_int_cmd(sensor, DVO_SETMIPISTATE, FLR_DVO_MIPI_STATE_OFF, 1) != R_SUCCESS) dev_warn(dev, "Could not set MIPI state to OFF");
...
static const struct of_device_id flir_boson_dt_ids[] = {{.compatible = "flir,boson-vvcam"}, {/* sentinel */}};
